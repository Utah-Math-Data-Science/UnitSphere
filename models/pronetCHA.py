...
class ProNet(nn.Module):
    ...
    def forward(self, batch_data):
        ...
        # if self.schull:
        #     if self.cutoff <= 0:
        #         edge_index = batch_data['ch_edge_index'].long()
        #     else:
        #         edge_index = batch_data['ch_cut_{}_edge_index'.format(int(self.cutoff))].long()
        # else:
        ...
        if self.schull:
            ...
            ch_pos_emb, ch_feature0, ch_feature1 = self.func_feature(z, ch_pos, batch_data, ch_edge_index, device)
            pos_emb = torch.cat((pos_emb, ch_pos_emb), dim=0)
            feature0 = torch.cat((feature0, ch_feature0), dim=0)
            feature1 = torch.cat((feature1, ch_feature1), dim=0)

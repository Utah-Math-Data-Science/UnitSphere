...
from pronet_features import d_angle_emb, d_theta_phi_emb
...
class ProNet(nn.Module):
    ...
    def __init__(
        ...
        if level == 'aminoacid':
            ...
            self.embedding = torch.nn.Linear(num_aa_type + num_bb_embs, hidden_channels//2)
        elif level == 'allatom':
            self.embedding = torch.nn.Linear(num_aa_type + num_bb_embs + num_side_chain_embs, hidden_channels)
        else:
            print('No supported model!')
        
        self.embedding_amino = torch.nn.Linear(11, hidden_channels//2)
        self.vec_emb = torch.nn.Linear(3, 3 * num_radial * num_spherical)
        self.ss_node_emb = torch.nn.Linear(num_ss_cls, hidden_channels//4)
    ...
    def forward(self, batch_data):
        ...
        x = self.embedding(x)
        x1 = self.embedding_amino(side_chain_embs)
        x = torch.cat((x, x1), 1)
        
        edge_index = batch_data.edge_index
        pos_emb, feature0, feature1 = self.func_feature(z, pos, batch_data, edge_index, device)

        
        pos_b = batch_data.coords_b_
        z_b = batch_data.ss_x.long()
        edge_index_b = radius_graph(pos_b, r=self.cutoff, 
        ...
        # Interaction blocks.
        for interaction_block, interaction_block_ss in zip(self.interaction_blocks, 
                                                           self.interaction_blocks_ss):
            ...
            x_ = self.node_out(x)
            x_ = scatter(x_, mapping_a_to_b, dim=0, dim_size=len(z_b), reduce='mean')
            x = torch.cat((ss_x, x_), 1)

...
class GVPNet(nn.Module):
    ...
    '''
    def __init__(self, num_rbf=16, schull=False):
    ...
    def forward(self, batch, scatter_mean=True, dense=True):
        ...
        if self.schull:
            ...
            ch_edge_s, ch_edge_v = batch.ch_edge_s, batch.ch_edge_v
            edge_index = torch.cat([edge_index, ch_edge_index], dim=1)
            edge_s = torch.cat([edge_s, ch_edge_s*0.2], dim=0)
            edge_v = torch.cat([edge_v, ch_edge_v*0.2], dim=0)
        else:
            edge_index = batch.edge_index
            edge_s, edge_v = batch.edge_s, batch.edge_v

...
class SimpleInteractionBlockCHA(torch.nn.Module):
    def __init__(
            self,
            hidden_channels,
            middle_channels,
        ...
        self.act = act
        self.act_hull = act_hull
        self.cha_rate = cha_rate
        self.conv1 = EdgeGraphConv(hidden_channels, hidden_channels)
        self.conv2 = EdgeGraphConv(hidden_channels, hidden_channels)
        self.conv_hull = EdgeGraphConv(hidden_channels, hidden_channels)
        self.lin1 = Linear(hidden_channels, int(cha_scale*hidden_channels*cha_rate))
    ...
    def forward(self, x, 
        ...
        feature_hull = torch.cat([feature_hull_1, feature_hull_2], dim=1)
        feature_hull = self.lin_feature_hull(feature_hull)
        h_hull = self.conv_hull(x, edge_index_hull, feature_hull)
        h_hull = self.lin_hull(h_hull)
        h_hull = self.act_hull(h_hull)

        h = self.lin_cat(torch.cat([h1, h2, h_hull], 1))

...
class SimpleInteractionBlock(torch.nn.Module):
    def __init__(
        ...
        if iscovhull:
            self.lin_feature1 = TwoLayerLinear(3, middle_channels, hidden_channels)
            self.lin_feature2 = TwoLayerLinear(2, middle_channels, hidden_channels)
        else:
...
class embHull(nn.Module):
    def __init__(
        ...
        self.emb_f1 = Linear(input_dim1, num_radial * num_spherical ** 2)
        self.emb_f2 = Linear(input_dim2, num_radial * num_spherical)
        self.act = act
    ...
    def forward(self, r, h, edge_index):
        ...
        fea1 = h
        #fea1 = self.act(self.emb_f1(h))
        #fea2 = self.act(self.emb_f2(fea2))
        return fea1, fea2

class mergeHull(nn.Module):
    def __init__(
        self,
        hidden_channels = 256,
        act = False
    ):
        super(mergeHull, self).__init__()
        self.func = Linear(hidden_channels*2, hidden_channels)
        self.act = act
...
class ComENet(nn.Module):
    ...
    def __init__(
        ...
        self.iscovhull = iscovhull
        if iscovhull:
            self.embhull = embHull(num_radial=num_radial, 
                                    num_spherical=num_spherical)
            self.mergehull = mergeHull(hidden_channels=hidden_channels)
        if sym is None:
        ...
        if iscovhull:
            self.interaction_blocks_hull = torch.nn.ModuleList(
            [
                SimpleInteractionBlock(
                    hidden_channels,
                    middle_channels,
    ...
    def _forward(self, data):
        ...
        if self.iscovhull:
            ...
            for interaction_block, interaction_block_hull in zip(self.interaction_blocks, self.interaction_blocks_hull):
                x = interaction_block(x, feature1, feature2, edge_index, batch)
                x = interaction_block_hull(x, feature_hull_1, 
                                            feature_hull_2, edge_index_hull, batch)
                # x_tilde = interaction_block_hull(x_tilde, feature_hull_1, 
                #                                  feature_hull_2, edge_index_hull, batch)
            # x = self.mergehull(x, x_tilde)

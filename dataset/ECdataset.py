    ...
    def process(self):
        ...
        with warnings.catch_warnings():
            ...
            for fileIter, curFile in tqdm(enumerate(fileList_)):
                ...
                try:
                    df, edges, s_graph_dict = get_pdb_info(pdb_id, case_id)
                    ...
                    continue
                # os.remove('/root/workspace/programme/SCHull/{}.pdb'.format(pdb_id))
                k += 1
                print('Processed {} / {} proteins'.format(k, len(fileList_)))
        data, slices = self.collate(data_list)
        torch.save((data, slices), self.processed_paths[0])
        print('Done!')
        print('Ratio of proteins processed: {:.2f}%', count/len(fileList_)*100)